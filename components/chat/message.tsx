...
import { BookOpenIcon, BrainCircuitIcon, GavelIcon, SearchIcon } from 'lucide-react';
...
export function PreviewMessage({
  ...
  return <motion.div className="w-full" initial={{
    ...
    opacity: 1
      ...
      <div className={`flex gap-4 ${message.role === 'user' ? 'bg-foreground/5 py-2 px-4 rounded-full text-sm w-fit ml-auto max-w-[90%]' : 'w-full'}`}>
        <div className={`flex-1 ${message.role === 'assistant' ? 'prose prose-zinc dark:prose-invert max-w-none' : ''}`}>
          {message.role === 'assistant' ? <div className="markdown-content text-foreground/90">
              <Markdown>{formatFollowUpQuestions(message.content)}</Markdown>
            </div> : <p className="text-primary">{message.content}</p>}
        </div>
      </div>
    </motion.div>;
}
export function ResearchProgress({
  ...
  const filteredProgress = progress.filter(step => {
    if (step.type === 'report') {
      ...
      return step.content.includes('Generating') || step.content.includes('Synthesizing');
    }
    return true;
  });
  if (!isLoading && filteredProgress.length === 0) {
    return <div className="h-full flex items-center justify-center text-neutral/50">
        <div className="text-center space-y-3">
          <BrainCircuitIcon className="w-12 h-12 mx-auto" />
          <p className="text-sm font-light tracking-wide">Desbloquea investigaciones potentes al alcance de tu mano</p>
        </div>
      </div>;
  }
  return <div className="h-full overflow-y-auto p-6 space-y-4">
      {filteredProgress.map((step, index) => <motion.div key={index} initial={{
      opacity: 0,
      y: 10
    }} animate={{
      opacity: 1,
      y: 0
          ...
          <div className="flex-1">
            ...
            {step.queries && <div className="mt-3 space-y-3">
                {step.queries.map((query, idx) => <motion.div key={idx} initial={{
            opacity: 0
          }} animate={{
            opacity: 1
          }} transition={{
